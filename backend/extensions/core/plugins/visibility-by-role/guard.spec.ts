...
const ROLES = [E_ROLE.MASTER, E_ROLE.ADMINISTRATOR, E_ROLE.MANAGER, E_ROLE.REGISTERED] as const;
const VISIBILITIES = [E_VISIBILITY.PUBLIC, E_VISIBILITY.SIGILOSO, undefined] as const;
...
describe('VisibilityByRoleGuard.canRead', () => {
  it.each(ROLES.flatMap((role) => VISIBILITIES.map((vis) => ({ role, vis }))))(
    ...
    ({ role, vis }) => {
      const result = VisibilityByRoleGuard.canRead(makeRow(vis), makeUser(role), TABLE);
  ...
  it('user undefined: comporta-se como nao-admin', () => {
    expect(VisibilityByRoleGuard.canRead(makeRow('PUBLIC'), undefined, TABLE)).toBe(true);
    expect(VisibilityByRoleGuard.canRead(makeRow('SIGILOSO'), undefined, TABLE)).toBe(false);
  });
});
...
describe('VisibilityByRoleGuard.adjustListQuery', () => {
  it('admin: query inalterada', () => {
    const q = { foo: 1 };
    expect(VisibilityByRoleGuard.adjustListQuery(q, makeUser(E_ROLE.MASTER), TABLE)).toEqual(q);
    expect(VisibilityByRoleGuard.adjustListQuery(q, makeUser(E_ROLE.ADMINISTRATOR), TABLE)).toEqual(q);
  });

  it('nao-admin: adiciona filtro data.visibility=PUBLIC', () => {
    expect(VisibilityByRoleGuard.adjustListQuery({ foo: 1 }, makeUser(E_ROLE.MANAGER), TABLE)).toEqual({
      foo: 1,
...
describe('VisibilityByRoleGuard.canWrite', () => {
  it('MASTER: allowed em qualquer caso', () => {
    expect(
      VisibilityByRoleGuard.canWrite(null, makeUser(E_ROLE.MASTER), TABLE, { visibility: 'SIGILOSO' }, 'create'),
    ).toEqual({ allowed: true });
  });

  it('MANAGER setando SIGILOSO: bloqueado', () => {
    const r = VisibilityByRoleGuard.canWrite(null, makeUser(E_ROLE.MANAGER), TABLE, { visibility: 'SIGILOSO' }, 'create');
    expect(r.allowed).toBe(false);
  });

  it('MANAGER setando PUBLIC: allowed', () => {
    expect(
      VisibilityByRoleGuard.canWrite(null, makeUser(E_ROLE.MANAGER), TABLE, { visibility: 'PUBLIC' }, 'create'),
    ).toEqual({ allowed: true });
  });

  it('payload null (delete): allowed para nao-admin (canRead ja barrou se necessario)', () => {
    expect(
      VisibilityByRoleGuard.canWrite(makeRow('PUBLIC'), makeUser(E_ROLE.MANAGER), TABLE, null, 'delete'),
    ).toEqual({ allowed: true });
  });
});
