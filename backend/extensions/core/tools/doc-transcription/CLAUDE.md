...
Todos blindados por `AuthenticationMiddleware` +
`RoleMiddleware([MASTER, ADMINISTRATOR])` +
`ExtensionActiveMiddleware({ pkg: 'core', type: TOOL, extensionId: 'doc-transcription' })`.
...
- `doc-transcription-config.model.ts` — model Mongoose singleton
  (`_id: 'singleton'`, collection `doc_transcription_config`) + helpers
...
4. Monta `FormData` (`document`, `documentType`, `responseFields` em JSON e, se
   houver, `model`) e faz `POST` à `apiUrl` com header `X-Api-Key`
