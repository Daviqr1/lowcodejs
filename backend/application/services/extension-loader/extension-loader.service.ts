...
import type { ExtensionAvailabilityKey } from '@application/repositories/extension/extension-contract.repository';
import { ExtensionContractRepository } from '@application/repositories/extension/extension-contract.repository';

import type { LoadExtensionsResult } from './extension-loader-contract.service';
import { ExtensionLoaderContractService } from './extension-loader-contract.service';
...
export default class ExtensionLoaderService implements ExtensionLoaderContractService {
  ...
  async load(): Promise<LoadExtensionsResult> {
    ...
    if (!existsSync(root)) {
      console.info(
        '[Extensions] Diretório extensions/ não encontrado, pulando.',
      );
    ...
    const unavailable =
      await this.repository.markUnavailableExcept(presentKeys);
