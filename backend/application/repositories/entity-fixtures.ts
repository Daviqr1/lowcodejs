...
/**
 * Builders de entidade usados pelos doubles `*-in-memory.repository.ts`: eles
 * guardam so o `_id` de uma ref populada, mas o contrato exige a entidade
 * inteira.
 *
 * Fica em `repositories/` e nao em `test/helpers/` porque os doubles sao
 * compilados junto de `application/**` pelo tsup, e `test/` nao e entry do
 * build — um import cruzado quebraria o artefato de producao.
 */
export class EntityFixtures {
  makePermission(id: string): IPermission {
    return {
      ...baseFields(id),
      name: id,
      slug: id,
  ...
  makeStorage(id: string): IStorage {
    return {
      ...baseFields(id),
      url: '',
      filename: id,
  ...
  makeTable(overrides: Partial<ITable> = {}): ITable {
    return {
      ...baseFields('table'),
      _schema: {},
      name: 'table',
  ...
  makeRow(overrides: Partial<IRow> = {}): IRow {
    return {
      _id: 'row',
      createdAt: new Date(),
      updatedAt: null,
