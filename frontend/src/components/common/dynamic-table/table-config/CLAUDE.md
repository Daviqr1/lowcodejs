...
| `table-field-visible-chips-limit.tsx`       | `TableFieldVisibleChipsLimit`       | Input numerico do limite de chips exibidos antes do resumo "+N" (dropdown/relacionamento/usuario multiple). Vazio = `null` (sem limite) — por isso nao usa o `FieldNumber` generico, que coage vazio para 0.          |
